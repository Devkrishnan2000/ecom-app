    ...
    render()
    {
        return(
            <div>
              ...
              {!this.state.noorder &&
                this.state.orderitem.map((result=><Orderitems key={result.oid} pid={result.pid} oid={result.oid} pname={result.pname} pimage={result.pimage} status={result.ostatus} loc={result.olocation}/>))
               }
               {
                this.state.noorder &&
