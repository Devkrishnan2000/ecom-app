...
export default class Apart extends Component {
  constructor(props) {
    super(props);
    this.state = {
      popname: "pname",
  ...
  onyearchange(e)
  {
    ...
    .get(
      "http://localhost:80/sem8project/ecom-app/ecom-app/api/getprodanalytics.php",
      { params: { curpartyear: e.target.value,curpartid:this.state.curpartid } }
    )
    .then((res) => {
  ...
  render() {
    return (
      <div className="revenue-parent-div">
        ...
        <T5ProdSale label={this.state.t5sales.map((res=>res.pname))} data={this.state.t5sales.map((res=>res.count))}></T5ProdSale>
        <h2 style={{ marginTop: 20 + "px"}}>TOP 5 HIGHLY REVIEWED PARTS</h2>
        <T5ProdSale label={this.state.t5rating.map((res=>res.pname))} data={this.state.t5rating.map((res=>res.rating))}></T5ProdSale>
        <h2 style={{ marginTop: 50 + "px",textAlign:"center"}}>PART WISE ANALYTICS</h2>
        <div style={{display:"flex",justifyContent:"center",marginTop:50+"px",marginBottom:50+"px"}}>
            <select name="prodselc" className="textbox" onChange={this.onpartchange}>
            <option value="" selected disabled hidden>
              Select Product
            </option>
            {this.state.parts.map((res=> <option key={res.pid} value={res.pid}>{res.pname}</option>))}
            </select>
            <select name="yearselc" className="textbox"  onChange={this.onyearchange}>
            <option value="" selected disabled hidden>
              Select Year
            </option>
            {this.state.years.map((res=> <option key={res.year} value={res.year}>{res.year}</option>))}
            </select>
        </div>
        <CurProd  type ="sales" label={this.state.months} data={this.state.curpartsales.map((res=>res.sum))}></CurProd>
        <h4 style={{textAlign:'center'}}>PART REVIEW ANALYSIS</h4>
        <ProdReview type ="sales" label={this.state.rating} data={this.state.currating.map((res=>res.count))} ></ProdReview>
        </div>
